...
fn test_single_agent_workflow() {
    ...
    print_success(&format!("Merged to unified, snapshot_id: {}", unified_snapshot_id.to_hex()));

    // Merge unified to staged layer
    print_info("Step 7.6: Merge unified to staged layer");
    let staged_snapshot_id = merge_to_staged(&env);
    print_success(&format!("Merged unified to staged, snapshot_id: {}", staged_snapshot_id.to_hex()));
...
fn test_agent_rejection() {
    ...
    for p in &approval_partitions {
        if let stratum::core::types::PartitionType::Approval(agent_id) = &p.partition_type {
            print_info(&format!("  - Partition: {}, history.len: {}, current: {}",
                agent_id, p.history.len(), truncate_id(&p.current_snapshot.to_hex())));
        }
    }
    ...
    assert!(!agent1_pending, "Agent-1 should not be pending after rejection");
