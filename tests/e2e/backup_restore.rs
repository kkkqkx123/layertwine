...
use stratum::api::{InitRequest, BackupRequest, RestoreRequest, CommitRequest};
...
fn test_backup_and_restore() {
    ...
    print_info("Step 8: Commit restored content");
    commit_changes(&env, "Restore from backup", "user-1");
    print_success("Restored content committed");

    // Verify log
    print_info("Step 9: Verify commit history");
    let log = get_log(&env, None);
    print_checkpoint_log(&log);
    assert_log_entry_count(&env, 5); // V1, V2, V3, modified, restored
...
fn test_multiple_backups() {
    ...
    let backups = vec![
        ("Backup 1", backup1.backup_id.clone(), content1),
        ("Backup 2", backup2.backup_id.clone(), content2),
        ("Backup 3", backup3.backup_id.clone(), content3),
    ];

    for (name, backup_id, expected_content) in &backups {
    ...
    for (name, backup_id, expected_content) in &backups {
        ...
        let restored_id = SnapshotId::from_hex(backup_id)
            .expect("Invalid backup ID");
        let restored_content = reconstruct_text(&env, &restored_id)
            .expect(&format!("Failed to reconstruct {}", name));
