...
use stratum::api::{InitRequest, CommitRequest, LogRequest, BranchCreateRequest, BranchSwitchRequest, MergeRequest};
...
fn test_create_and_switch_branch() {
    ...
    let create_response = env.api.branch_create(BranchCreateRequest {
    ...
    let list_response = env.api.branch_list()
        .expect("Failed to list branches");

    print_info("Available branches:");
    for branch in &list_response.branches {
        let is_current = if branch.is_current { " (current)" } else { "" };
        print_info(&format!("  - {}{}: head={}", branch.name, is_current, &branch.head[..12]));
    }

    assert!(list_response.branches.iter().any(|b| b.name == "feature"),
        "feature branch should exist");
    assert!(list_response.branches.iter().any(|b| b.name == "main"),
    ...
    print_info(&format!("  Branch head: {}", &switch_response.checkpoint_id[..12]));
    ...
    env.api.branch_switch(BranchSwitchRequest {
        name: "main".to_string(),
    }).expect("Failed to switch branch");
    ...
    assert_log_entry_count(&env, 1, "Main branch should only have 1 commit");
    ...
    if !staged_partitions.is_empty() {
        ...
        if let Some(content) = reconstruct_text(&env, &snapshot_id) {
            ...
            assert_eq!(content, base_content, "Main branch content should not have feature addition");
        }
    }
...
fn test_branch_isolation() {
    ...
    env.api.branch_create(BranchCreateRequest {
        name: "feature".to_string(),
    }).expect("Failed to create branch");
    ...
    assert!(feature_reconstructed.is_some(), "Failed to reconstruct feature content");
    ...
    assert_eq!(actual_feature_content, feature_content,
    ...
    assert!(main_reconstructed.is_some(), "Failed to reconstruct main content");

    let actual_main_content = main_reconstructed.unwrap();
    print_info("Main branch content:");
    print_file_content(&actual_main_content, 5);
    assert_eq!(actual_main_content, main_content,
...
fn test_simple_branch_merge() {
    ...
    print_success(&format!("Merge completed, checkpoint_id: {}", &merge_response.checkpoint_id[..12]));
    print_info(&format!("  Source: {}, Target: {}",
    ...
    assert!(merged_content.is_some(), "Failed to reconstruct merged content");

    let actual_merged_content = merged_content.unwrap();
    print_info("Merged content:");
    print_file_content(&actual_merged_content, 5);
    assert_eq!(actual_merged_content, feature_content, "Merged content mismatch");

    // Verify log has merge commit
    print_info("Step 7: Verify merge in log");
    let log = get_log(&env, None);
    print_checkpoint_log(&log);
    assert_log_entry_count(&env, 3, "Should have initial, feature, and merge commits");
    assert_log_contains(&env, "Merge feature branch");
...
fn test_multiple_branches() {
    ...
    assert_eq!(list_response.branches.len(), 4, "Should have 4 branches (main + 3 features)");
    ...
    for branch_name in &["main", "feature-1", "feature-2", "feature-3"] {
        ...
        let snapshot_id = SnapshotId::from_hex(&staged_partitions[0].current_snapshot)
            .expect("Invalid snapshot ID");
        let actual_content = reconstruct_text(&env, &snapshot_id)
            .expect("Failed to reconstruct content");

        assert_eq!(actual_content, expected_content,
            "Content mismatch on branch {}", branch_name);
...
fn assert_log_entry_count(env: &TestEnvironment, expected: usize) {
    let log = get_log(env, None);
    assert_eq!(log.len(), expected,
        "Log should have {} entries, but has {}", expected, log.len());
}

fn assert_log_entry_count(env: &TestEnvironment, expected: usize, message: &str) {
    let log = get_log(env, None);
    assert_eq!(log.len(), expected, "{}", message);
}