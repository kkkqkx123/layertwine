...
use stratum::api::{InitRequest, CommitRequest, LogRequest, BranchCreateRequest, BranchSwitchRequest, MergeRequest, ApiService};
...
fn test_simple_branch_merge() {
    ...
    print_info(&format!("  Partitions before merge: {}", status_before.partitions.len()));
    ...
    print_info(&format!("  Partitions after merge: {}", status_after.partitions.len()));

    for p in &status_after.partitions {
        print_info(&format!("    - Layer: {}, Name: {}, Snapshot: {}",
            p.layer, p.name, &p.current_snapshot[..16]));
    }
    ...
    print_info(&format!("Total log entries (excluding root): {}", log.len()));
